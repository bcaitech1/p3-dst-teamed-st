...
class TRADEBERT(nn.Module):
    ...
    def tie_weight(self):
        '''
        decoder의 weight를 encoder의 weight로 연결해주는 메소드입니다.
        즉, decoder weight가 encoder weight를 참조함으로써 같은 값을 공유하게 되는 것입니다.
        :return:
        '''
    ...
    def forward(
        ...
        '''
        TRADEBERT의 forward가 진행됩니다. 이 때, 1개의 인코더, 1개의 디코더에서
        output이 총 2개가 나오게 됩니다.
        :param input_ids: Tensor
        :param token_type_ids: Tensor
...
class BERTEncoder(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask):
        '''
        TRADEBERT의 encoder 부분의 BERT 모듈입니다.
        :param input_ids: Tensor
        :param token_type_ids: Tensor
        :param attention_mask: Tensor
        :return: output.last_hidden_state: Tensor, pooled_output:Tensor
        '''
        output = self.bert(
            input_ids=input_ids,
            token_type_ids=token_type_ids,
