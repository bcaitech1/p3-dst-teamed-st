...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_lr", type=int, default=0.00002)
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument("--pickle_name", type=str, default='bongjin')
    ...
    new_train_data_file = f"{args.data_dir}/new_train.json"
    train_data, dev_data, dev_labels = load_dataset([train_data_file, new_train_data_file])
    print(len(train_data))
    print(len(dev_data))

    # exmaple 10개 1개 씩 추출 후 실험 ㄱㄱㄱㄱㄱ
    ...
    #pickle name
    pickle_name = args.arch_name + 'new'
    if not os.path.exists(os.path.join(args.data_dir, f"new_train_somdst_features_{pickle_name}.pkl")):
        print("Cached Input Features not Found.\nLoad data and save.")

        # Extracting Featrues
        train_features = processor.convert_examples_to_features(train_examples)
        print("Save Data")
        with open(os.path.join(args.data_dir, f"new_train_somdst_features_{pickle_name}.pkl"), "wb") as f:
            pickle.dump(train_features, f)
        with open(os.path.join(args.data_dir, f"new_dev_somdst_examples_{pickle_name}.pkl"), "wb") as f:
            pickle.dump(dev_examples, f)
        with open(os.path.join(args.data_dir, f"new_dev_somdst_labels_{pickle_name}.pkl"), "wb") as f:
            pickle.dump(dev_labels, f)
    else:
        print("Cached Input Features Found.\nLoad data from Cached")
        with open(os.path.join(args.data_dir, f"new_train_somdst_features_{pickle_name}.pkl"), "rb") as f:
            train_features = pickle.load(f)
        with open(os.path.join(args.data_dir, f"new_dev_somdst_examples_{pickle_name}.pkl"), "rb") as f:
            dev_examples = pickle.load(f)
        with open(os.path.join(args.data_dir, f"new_dev_somdst_labels_{pickle_name}.pkl"), "rb") as f:
    ...
    if args.model_name:
        ...
        ckpt = torch.load(os.path.join(args.model_dir, f"base-model-{args.ckpt}.bin"))
...
#                 print(f"DOMAIN!!!! : {domain_scores} \n, STATE !!!!! : {state_scores} \n, GENENEEN !!!! :  {gen_scores}")
            ...
            if step % 50 == 0:
                ...
                wandb.log({'mean_loss' : loss_1.item(), 'state_loss' : loss_2.item(), 'dom_loss' : loss_3.item(), 'learning_rate' : args.learning_rate})
                batch_loss = []
        ...
        torch.save(
            model.state_dict(), f"{args.model_dir}/final-model-{epoch + args.ckpt}.bin"
        )
