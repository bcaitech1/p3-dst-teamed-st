...
class TRADEPreprocessorTest(DSTPreprocessor):
    ...
    def _convert_example_to_feature(self, example):
        ...
        if len(dialogue_id) + len(state_id) > self.max_seq_length:
            gap = len(dialogue_id) + len(state_id) - self.max_seq_length
            dialogue_id = dialogue_id[gap:]

        dialogue_segment_id = [0] * len(dialogue_id)
        state_segment_id = [1] * len(state_id)
        input_id = dialogue_id + state_id
        segment_id = dialogue_segment_id + state_segment_id
