...
if __name__ == "__main__":
    ...
    for epoch in range(n_epochs):
        ...
        ##############################
        coco_predictions = inference(model, coco_dev_examples, processor, device)
        coco_eval_result = _evaluation(coco_predictions, coco_dev_labels, slot_meta)
        for k, v in coco_eval_result.items():
            print(f"coco data -> {k}: {v}")
        if save:
            wandb.log({
                "coco_joint_acc": coco_eval_result['joint_goal_accuracy'],
                "coco_turn_slot_acc": coco_eval_result['turn_slot_accuracy'],
        ...
        if epoch == 30 or epoch == 40 or epoch == 49:
